...
public class AuthService : IAuthService
{
  ...
  public async Task<bool> LogoutAsync(string userId)
  {
    try
    {
      ...
      var key1Deleted = await _redisDb.KeyDeleteAsync($"user:{userId}:refreshToken");

      var key2Deleted = true;
      if (!refreshToken.IsNullOrEmpty)
      {
        key2Deleted = await _redisDb.KeyDeleteAsync($"refreshToken:{refreshToken}");
      }

      return key1Deleted && key2Deleted;
    }
    catch (Exception ex)
    {
      _logger.LogError(ex, "Ошибка при выполнении Logout для пользователя {UserId}", userId);
      return false;
    }
  }
}